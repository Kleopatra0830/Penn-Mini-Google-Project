...
public class HW6Test extends GenericTest {
  ...
  void runTests(Set<String> tests) throws Exception {
    ...
    // add word count test
    if (tests.contains("foldbykey")) try {
    	String ctx = "Stars shine brightly in the night sky , and among those stars , some stars shine brighter than other stars , making them stand out in the vast universe of stars .";
    	String[] arg = ctx.split(" ");
    	String response = FlameSubmit.submit("localhost:9000", "tests/flame-foldbykey.jar", "cis5550.test.FlameWordCount", arg);
